...
4. If you want to execute the model experimentation, you have to run `python -m src.fine_tuning.model_trainingloraqlorahiper`. If available, it uses the GPU to train the models.
5. If you want to execute the model testing, run `python -m src.fine_tuning.model_testing`.
...
- `fine_tuning`: Contains all the scripts related to the second deliverable (`augmentation_zone.py`, `fine_tuning_zone.py`, `model_testing.py`, `model_trainingloraqlorahiper.py`. `split_zone.py`, `training_dataset_zone.py`). It also contains a folder `image_experiments` with the results of model training and model experimentation.
