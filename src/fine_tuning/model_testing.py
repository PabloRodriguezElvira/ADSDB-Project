...
def download_top_5_with_scores(model_zs, model_qlora, dataset, sample_idx=None):
    ...
    print(f"Procesando bÇ§squeda para receta idx {sample_idx}...")
...
def get_top_k_stats(model, dataloader, device):
    """Compute top-k retrieval counts for k=1..5 plus out-of-5."""
    model.eval()
...
def plot_top_k_retrieval(stats_zs, stats_qlora):
    """Plot a side-by-side top-k retrieval bar chart."""
...
def plot_final_test_evolution(history, loss_zs_baseline):
    """Plot evaluation loss over epochs against a baseline."""
...
def generate_pca(model, model_name, dataset, device, filename):
    """Generate and save a PCA plot for image/text embeddings."""
    model.eval()
